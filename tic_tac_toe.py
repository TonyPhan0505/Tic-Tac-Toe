      ...
      if not self.continue_game:
         ...
         if self.is_col_win():
            for r in range(self.board_size):
               flasher = self.board[r][self.col_win]
               flashers.append(flasher)
         elif self.is_row_win():
            flashers = self.board[self.row_win]
         elif self.is_diag_win():
            if self.diag_win == 0:
               for r in range(self.board_size):
                  for c in range(self.board_size):
                     if r == c:
                        flasher = self.board[r][c]
                        flashers.append(flasher)
            else:
               for r in range(self.board_size):
                  for c in range(self.board_size):
         ...
         flasher = random.choice(flashers)
